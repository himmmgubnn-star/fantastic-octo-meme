/*
 * platform.h — portable OS abstractions used by the Win32 layer.
 *
 * Airlock targets "all Linux environments", which includes Android. Android
 * ships the Bionic libc, which exposes the same POSIX surface Linux does, so
 * a single implementation (src/port/posix.c) serves both. This header is the
 * seam: anything OS-specific goes through here, and porting to a new OS means
 * providing one more implementation file — nothing else changes.
 *
 * SPDX-License-Identifier: MIT
 */
#ifndef AIRLOCK_PLATFORM_H
#define AIRLOCK_PLATFORM_H

#include <stddef.h>
#include <stdint.h>

#include "airlock.h"

/* ---- Clock & sleep ------------------------------------------------------- */

/* Sleep for `ms` milliseconds (never busy-waits; nanosleep on POSIX). */
void airlock_sleep_ms(uint32_t ms);

/* Milliseconds on a monotonic clock (unaffected by wall-clock changes). */
uint64_t airlock_monotonic_ms(void);

/* High-resolution performance counter for QueryPerformanceCounter. The
 * absolute value is meaningless; ratios of two readings have units of seconds
 * when divided by airlock_perf_frequency(). */
uint64_t airlock_perf_counter(void);

/* Nominal frequency of airlock_perf_counter() in counts per second. */
uint64_t airlock_perf_frequency(void);

/* Current time as a 100ns-interval FILETIME since 1601-01-01 UTC, and the
 * local-time breakdown in FILETIME's (year/month/day/...) fields. */
void airlock_system_time_as_filetime(uint64_t *out_100ns_since_1601);

/* ---- Process / thread identity ------------------------------------------- */

/* OS process ID (airlock_getpid) and OS thread ID (gettid on Linux/Bionic). */
uint32_t airlock_getpid(void);
uint32_t airlock_gettid(void);

/* ---- File mapping (zero-copy reads) --------------------------------------- */

/* A read-only memory mapping of a file. `data`/`size` are valid until
 * airlock_unmap_file(). */
typedef struct airlock_mapped_file {
    const void *data;
    size_t      size;
    void       *_region;   /* implementation detail */
    size_t      _region_len;
} airlock_mapped_file_t;

airlock_status_t airlock_map_file(const char *path, airlock_mapped_file_t *out);
void airlock_unmap_file(airlock_mapped_file_t *mf);

#endif /* AIRLOCK_PLATFORM_H */
