 ...
 * timeGetTime, DWM frame pacing). Airlock provides a calibrated monotonic
 ...
 */
#ifndef AIRLOCK_TIMER_H
#define AIRLOCK_TIMER_H
...
uint64_t airlock_timer_us(void);
uint64_t airlock_timer_ms(void);
 ...
 * Returns AIRLOCK_OK, or AIRLOCK_ERR_INVALID_ARGUMENT for a past deadline. */
...
#define AIRLOCK_FRAMETIME_HISTORY 240

typedef struct airlock_frametime {
    uint64_t history[AIRLOCK_FRAMETIME_HISTORY]; /* frame durations, ns */
    size_t   count;
    size_t   pos;
...
typedef struct airlock_frametime_stats {
