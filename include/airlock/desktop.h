/*
 * desktop.h — Linux desktop integration for Windows applications.
 *
 * .desktop shortcuts, MIME file associations, notifications, an in-process
 * clipboard, and a default-browser hook. The goal is for a Windows program
 * running under Airlock to feel native on a Linux desktop.
 *
 * SPDX-License-Identifier: MIT
 */
#ifndef AIRLOCK_DESKTOP_H
#define AIRLOCK_DESKTOP_H

#include <stddef.h>

#include "airlock.h"

typedef struct airlock_desktop_entry {
    char name[128];
    char exec[512];
    char icon[256];
    char mime[128];
    char categories[128];
} airlock_desktop_entry_t;

/* Write a freedesktop.org .desktop file into `dir`. */
airlock_status_t airlock_desktop_write_shortcut(const char *dir,
                                              const airlock_desktop_entry_t *e);

/* Append a MIME association (`mime=desktop-id`) to mimeapps.list in `dir`. */
airlock_status_t airlock_desktop_write_mime(const char *dir, const char *mime,
                                          const char *desktop_id);

airlock_status_t airlock_desktop_notify(const char *summary, const char *body);

airlock_status_t airlock_clipboard_set(const char *text);
airlock_status_t airlock_clipboard_get(char *buf, size_t cap);

/* Record the URL a Windows app asked to open (does not spawn a browser). */
airlock_status_t airlock_desktop_open_url(const char *url);
const char *airlock_desktop_last_url(void);

#endif /* AIRLOCK_DESKTOP_H */
