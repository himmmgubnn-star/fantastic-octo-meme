...
static int add_unique_dll(cellar_inspect_t *ins, const char *dll)
{
    size_t i;
    if (!dll || !*dll)
        return 0;
    for (i = 0; i < ins->unique_dll_count; i++)
        if (strcasecmp(ins->unique_dlls[i], dll) == 0)
            return 0;
    if (ins->unique_dll_count >= CELLAR_INSPECT_MAX_DLLS)
        return 0;
...
static void classify_dll(cellar_inspect_t *ins, const char *dll)
{
...
static void walk_resources(const cellar_image_t *img, uint32_t root,
                           uint32_t off, int depth, int *found_manifest)
{
    ...
    if (depth > 3 || !found_manifest)
        return;
    if (!cellar_image_read(img, root + off, hdr, sizeof hdr))
        return;
    named = cellar_le16(hdr + 12);
    ids   = cellar_le16(hdr + 14);
    ...
    for (i = 0; i < n; i++) {
        ...
        if (!cellar_image_read(img, root + off + 16u + (uint32_t)i * 8u,
                               ent, sizeof ent))
            break;
        name   = cellar_le32(ent);
        offset = cellar_le32(ent + 4);
                                     ...
                                     cellar_inspect_t *out)
{
    ...
    size_t i;
    const cellar_pe_data_directory_t *dd;
    ...
    ins.is_64bit  = (img->opt.magic == CELLAR_PE_MAGIC_PE32_PLUS);
    ...
    ins.tls_rva = dd[CELLAR_PE_DIR_TLS].virtual_address;
    ins.tls_size = dd[CELLAR_PE_DIR_TLS].size;
    ins.has_tls = (ins.tls_rva != 0 && ins.tls_size != 0);

    ins.resource_rva = dd[CELLAR_PE_DIR_RESOURCE].virtual_address;
    ins.resource_size = dd[CELLAR_PE_DIR_RESOURCE].size;
    ...
    ins.com_rva = dd[CELLAR_PE_DIR_COM].virtual_address;
    ins.com_size = dd[CELLAR_PE_DIR_COM].size;
    ...
    ins.has_delay_imports = (dd[CELLAR_PE_DIR_DELAYIMPORT].virtual_address != 0 &&
                             dd[CELLAR_PE_DIR_DELAYIMPORT].size != 0);
