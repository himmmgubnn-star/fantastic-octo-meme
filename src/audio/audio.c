...
{
    const char *p = getenv("AIRLOCK_WAV_OUT");
    return (p && *p) ? p : "airlock-out.wav";
}

static airlock_status_t wav_open(struct airlock_audio_device *dev,
...
static airlock_status_t wav_write(struct airlock_audio_device *dev,
...
static airlock_status_t wav_reset(struct airlock_audio_device *dev)
{
...
static uint64_t wav_position_ms(const struct airlock_audio_device *dev)
{
...
static airlock_status_t wav_close(struct airlock_audio_device *dev)
{
    wav_sink_t *s = dev->impl;
    if (!s)
        return AIRLOCK_OK;
...
static const airlock_audio_backend_t wav_backend = {
    "wav-sink",
    wav_open,
    wav_write,
    wav_reset,
...
#ifdef AIRLOCK_AUDIO_ALSA
extern const airlock_audio_backend_t airlock_alsa_backend;
#endif

const airlock_audio_backend_t *airlock_audio_default_backend(void)
{
#ifdef AIRLOCK_AUDIO_ALSA
    return &airlock_alsa_backend;
#else
    return &wav_backend;
#endif
}
                                   ...
                                   const void *data, uint32_t bytes)
{
    ...
    if (st == AIRLOCK_OK) {
        airlock_perf_trace("audio.write", bytes);
        AIRLOCK_TRACE(AIRLOCK_TRACE_AUDIO, "write %u bytes -> %s",
