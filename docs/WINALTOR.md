# Winaltor feature layer

**Cellar** is the compatibility engine; **Winaltor** is the product layer on top
of it: one isolated environment per app, guided setup, an app library,
...
```sh
cellar app add "My Game" setup.exe --kind exe --launch drive_c/Games/My/game.exe
cellar app list
cellar app show "My Game"
cellar app set "My Game" gfx_backend Vulkan
cellar app doctor "My Game"
cellar app snapshot "My Game" before-update
cellar app rollback "My Game" before-update
cellar app support "My Game" support.zip
cellar app diagnose wine.log
cellar app safety "My Game"
cellar profile list|show|apply|export|import "My Game" [label|path]
cellar device report
cellar container create "My Game" setup.exe --launch game.exe
```

A workspace is a directory under `$WINALTOR_ROOT` (defaults to the Cellar
prefix directory):
...
| Diagnostics | Compatibility test button (`cellar test` / workspace doctor) | **Done** |
...
| Developer | Automated compatibility tests | **Done** (`cellar test`, `make test`) |
...
3. **Resource-bound.** The workspace manager only reads small metadata files;
   large caches are deleted/moved with the existing `CBK1`-style code paths.
